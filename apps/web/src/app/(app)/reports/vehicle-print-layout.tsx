...
import { VehicleLog, VehicleSummary, Center, Mission } from '@/lib/api';
import { assetUrl } from '@/lib/utils';

export interface VehiclePrintLayoutProps {
  logs: VehicleLog[];
...
export const VehiclePrintLayout: React.FC<VehiclePrintLayoutProps> = ({
  logs,
  summary,
  selectedCenterId,
  centers,
        ...
        <table className="w-full border-collapse border border-black text-[11px] mb-6">
          <thead>
            <tr className="bg-[#e2edd8] border-b border-black text-slate-900 font-bold text-center">
              ...
              <th className="border border-black py-2 px-1 w-[14%]">ประเภทยานพาหนะ</th>
            ...
            {logs.length === 0 ? (
              <tr>
                <td colSpan={8} className="border border-black py-8 text-center text-slate-500">
                  - ไม่พบข้อมูลรายการสถิติการซ่อมและบริการยานพาหนะ -
                </td>
              </tr>
            ) : (
              logs.map((log, index) => {
